...
export function validateSegments(segments: readonly StoryboardSegment[], maxSegments: number): void {
  ...
  for (const seg of segments) {
    ...
    for (const key of IMAGE_KEYS) {
      if (typeof (image as unknown as Record<string, unknown>)[key] !== 'string') {
    ...
    for (const key of VIDEO_KEYS) {
      if (typeof (video as unknown as Record<string, unknown>)[key] !== 'string') {
...
export function renderStoryboardMarkdown(doc: StoryboardDoc): string {
  const lines: string[] = []
  ...
  for (const seg of doc.segments) {
    ...
    if (seg.image_prompt.composition !== undefined) lines.push(`- 构图：${seg.image_prompt.composition}`)
    if (seg.image_prompt.lighting !== undefined) lines.push(`- 光线：${seg.image_prompt.lighting}`)
    if (seg.image_prompt.grading !== undefined) lines.push(`- 色调：${seg.image_prompt.grading}`)
    if (seg.image_prompt.keep !== undefined) lines.push(`- 保持：${seg.image_prompt.keep}`)
    if (seg.image_prompt.change !== undefined) lines.push(`- 变更：${seg.image_prompt.change}`)
    if (seg.image_prompt.negative !== undefined) lines.push(`- 负面提示词：${seg.image_prompt.negative}`)
    lines.push('')
    ...
    if (seg.video_prompt.camera !== undefined) lines.push(`- 运镜：${seg.video_prompt.camera}`)
    if (seg.video_prompt.expression !== undefined) lines.push(`- 表情：${seg.video_prompt.expression}`)
    if (seg.video_prompt.body !== undefined) lines.push(`- 肢体：${seg.video_prompt.body}`)
    if (seg.video_prompt.negative !== undefined) lines.push(`- 负面提示词：${seg.video_prompt.negative}`)
    if (seg.transition !== undefined) lines.push(`- 衔接：${seg.transition}`)
    lines.push('')
  }
  return lines.join('\n')
}

/** Compact model-facing summary of the exported storyboard (output-shape aware). */
interface ExportSummaryValue {
  title: string
  json_path: string
  markdown_path: string
  segment_count: number
  segments: Array<{ segment: string; beat: string }>
}

function formatExportSummary(value: ExportSummaryValue): string {
  const lines = [
...
export function apply(ctx: Context, config: Config | undefined): void {
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      validateSegments(args.segments as StoryboardSegment[], maxSegments)
      const doc: StoryboardDoc = {
        ...
        segments: args.segments as StoryboardSegment[],
        progress: {},
      }
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      let doc: StoryboardDoc
      try {
        doc = JSON.parse(raw) as StoryboardDoc
      } catch {
