...
const name = "tool-screenwriter";
/** Services required before the tools can mount. */
const inject = ["tools", "fs"];
const Config = z.object({ maxSegments: z.natural().max(200) });
...
function validateSegments(segments, maxSegments) {
	if (segments.length === 0) throw new Error("storyboard_export: segments 不能为空");
	if (segments.length > maxSegments) throw new Error(`storyboard_export: 分段数 ${String(segments.length)} 超过上限 ${String(maxSegments)}`);
	const seen = /* @__PURE__ */ new Set();
	for (const seg of segments) {
		if (typeof seg.segment !== "string" || seg.segment.trim() === "") throw new Error("storyboard_export: 存在缺少 segment 编号的分段（如 A、B、C）");
		if (seen.has(seg.segment)) throw new Error(`storyboard_export: 分段编号 "${seg.segment}" 重复`);
		seen.add(seg.segment);
		if (typeof seg.beat !== "string" || seg.beat.trim() === "") throw new Error(`storyboard_export: 段 ${seg.segment} 缺少 beat 剧情说明`);
		const image = seg.image_prompt;
		if (typeof image !== "object") throw new Error(`storyboard_export: 段 ${seg.segment} 缺少 image_prompt（画面四要素）`);
		for (const key of IMAGE_KEYS) if (typeof image[key] !== "string") throw new Error(`storyboard_export: 段 ${seg.segment} 的 image_prompt 缺少 "${key}"（画面四要素：时间/主体/场景/风格）`);
		const video = seg.video_prompt;
		if (typeof video !== "object") throw new Error(`storyboard_export: 段 ${seg.segment} 缺少 video_prompt（场景四要素）`);
		for (const key of VIDEO_KEYS) if (typeof video[key] !== "string") throw new Error(`storyboard_export: 段 ${seg.segment} 的 video_prompt 缺少 "${key}"（场景四要素：主体/动作/效果/节奏）`);
	}
}
/** Render the human-readable 分镜表 Markdown. */
function renderStoryboardMarkdown(doc) {
	const lines = [];
	lines.push(`# ${doc.title} · 分镜表`);
	lines.push("");
	if (doc.style_anchor !== void 0 && doc.style_anchor !== "") lines.push(`**风格锚点**：${doc.style_anchor}`);
	if (doc.total_duration_sec !== void 0) lines.push(`**预估总时长**：${String(doc.total_duration_sec)} 秒（${String(doc.segments.length)} 段）`);
	...
	ctx.tools.register(defineTool({
		...
		parameters: {
			title: {
				type: "string",
				required: true,
				description: "短剧标题（用于文件名与分镜表标题）。"
			},
			style_anchor: {
				type: "string",
				description: "全片风格锚点一句话（色调/光线/美术基调）。"
			},
			total_duration_sec: {
				type: "integer",
				description: "预估总时长（秒）。"
			...
			segments: {
				...
				items: {
					...
					properties: {
						...
						duration_sec: {
							type: "integer",
							description: "本段视频时长（秒）。"
						},
						img2img_from: {
							type: "string",
							description: "图生图来源（如 S 锚点图、A）。"
						},
						image_prompt: {
							type: "object",
							required: true,
		...
		output: {
			...
			render: (_args, value) => [{
				type: "text",
				text: formatExportSummary(value)
			}]
		},
		...
		async execute(args, exec) {
			...
			const doc = {
				...
				...args.style_anchor !== void 0 && args.style_anchor !== "" ? { style_anchor: args.style_anchor } : {},
				...args.total_duration_sec !== void 0 ? { total_duration_sec: args.total_duration_sec } : {},
			...
			const jsonOutcome = await ctx.fs.writeText(jsonTarget, jsonText, void 0, exec.signal);
			const mdOutcome = await ctx.fs.writeText(mdTarget, mdText, void 0, exec.signal);
			ctx.emit("fs/observed", jsonTarget, {
				kind: "present",
				version: jsonOutcome.version
			}, exec);
			ctx.emit("fs/observed", mdTarget, {
				kind: "present",
				version: mdOutcome.version
			}, exec);
			return {
				title: doc.title,
	...
	ctx.tools.register(defineTool({
		...
		parameters: {
			storyboard_path: {
				type: "string",
				required: true,
				description: "storyboard JSON 文件路径（storyboard_export 返回的 json_path）。"
			},
			segment: {
				type: "string",
				required: true,
				description: "分段编号，如 A。"
			},
			status: {
				type: "string",
				required: true,
			...
			note: {
				type: "string",
				description: "可选说明（如重做原因）。"
			},
			image_path: {
				type: "string",
				description: "可选：已确认图片的文件路径。"
			}
		},
		output: {
			schema: {
		...
		async execute(args, exec) {
			...
			if (info === void 0) throw new Error(`storyboard_status: 找不到分镜表文件 "${args.storyboard_path}"，请先运行 storyboard_export`);
			if (info.type !== "file") throw new Error(`storyboard_status: "${args.storyboard_path}" 不是文件`);
			...
			if (doc.schema !== SCHEMA_TAG) throw new Error(`storyboard_status: "${args.storyboard_path}" 不是 dsh-screenwriter 分镜表（schema=${String(doc.schema)}）`);
			if (!doc.segments.some((seg) => seg.segment === args.segment)) throw new Error(`storyboard_status: 分镜表中不存在段 "${args.segment}"（现有：${doc.segments.map((seg) => seg.segment).join(", ")}）`);
			doc.progress[args.segment] = {
				status: args.status,
				...args.note !== void 0 ? { note: args.note } : {},
				...args.image_path !== void 0 ? { image_path: args.image_path } : {}
			};
			...
			const mdTarget = await ctx.fs.resolve(`storyboard-${slugify(doc.title)}.md`, resolveOpts);
			const mdOutcome = await ctx.fs.writeText(mdTarget, renderStoryboardMarkdown(doc), void 0, exec.signal);
			ctx.emit("fs/observed", mdTarget, {
				kind: "present",
				version: mdOutcome.version
			}, exec);
			const progress = doc.segments.map((seg) => {
				...
				return {
					...
					...entry?.image_path !== void 0 ? { image_path: entry.image_path } : {}
				};
			});
			return {
...
//#endregion
export { Config, apply, apply as default, inject, name, renderStoryboardMarkdown, slugify, validateSegments };
